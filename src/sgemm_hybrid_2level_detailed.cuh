...
#include "cublas_helpers.cuh" // For stream-aware matrix_add_gpu etc.
#include "helper_cuda.h" // For CUDA_CHECK, CUBLAS_CHECK (now direct due to include_directories)
#include "sgemm_strassen_fused.cuh" // For sgemm_strassen_1level_fused_vP
#include <cublas_v2.h>
...
void
sgemm_strassen_hybrid_2level_detailed(int M,
                                      int N,
                                      int K,
    ...
    if (M % 4 != 0 || N % 4 != 0 || K % 4 != 0) {
        ...
        snprintf(err_msg,
                 sizeof(err_msg),
                 "sgemm_strassen_hybrid_2level_detailed: M, N, K must be divisible by 4. Got M=%d, "
                 "N=%d, K=%d",
                 M,
                 N,
                 K);
    ...
    if (M < 4 || N < 4 || K < 4) { // Practical minimum for 2-level
        ...
        snprintf(
            err_msg,
            sizeof(err_msg),
            "sgemm_strassen_hybrid_2level_detailed: M, N, K must be >= 4. Got M=%d, N=%d, K=%d",
            M,
            N,
            K);
        fprintf(stderr, "%s\n", err_msg);
    ...
    cudaStream_t main_stream =
        streams[0]; // Use streams[0] for S/T, C setup, D2H, and some M products.
    ...
    CUDA_CHECK(
        cudaMallocAsync((void**)&d_A_full, (size_t)M * lda_host_full * sizeof(float), main_stream));
    CUDA_CHECK(
        cudaMallocAsync((void**)&d_B_full, (size_t)K * ldb_host_full * sizeof(float), main_stream));
    CUDA_CHECK(
        cudaMallocAsync((void**)&d_C_full, (size_t)M * ldc_host_full * sizeof(float), main_stream));
    ...
    if (host_beta_overall == 0.0f) {
        CUDA_CHECK(
            cudaMemsetAsync(d_C_full, 0, (size_t)M * ldc_host_full * sizeof(float), main_stream));
    } else {
        ...
        for (int r = 0; r < M; ++r) {
            for (int c = 0; c < N; ++c) {
                C_temp_scaled_host[r * ldc_host_full + c] = C_host_result[r * ldc_host_full + c]
                                                            * host_beta_overall;
            }
        }
        CUDA_CHECK(cudaMemcpy2DAsync(d_C_full,
                                     ldc_host_full * sizeof(float),
                                     C_temp_scaled_host.data(),
                                     ldc_host_full * sizeof(float),
                                     N * sizeof(float),
                                     M,
                                     cudaMemcpyHostToDevice,
    ...
    float *M0_d = nullptr, *M1_d = nullptr, *M2_d = nullptr, *M3_d = nullptr, *M4_d = nullptr,
          *M5_d = nullptr, *M6_d = nullptr;
    ...
    CUDA_CHECK(
        cudaMallocAsync((void**)&M0_d,
                        m_matrix_bytes,
                        main_stream)); // M_i can also use specific streams if alloc is bottleneck
    ...
    const float* S2_ptr_d = A00_d;
    const int ld_S2 = lda_host_full;
    const float* S3_ptr_d = A11_d;
    const int ld_S3 = lda_host_full;
    const float* T1_ptr_d = B00_d;
    const int ld_T1 = ldb_host_full;
    const float* T4_ptr_d = B11_d;
    const int ld_T4 = ldb_host_full;
    ...
    CUDA_CHECK(cudaStreamSynchronize(
        main_stream)); // Ensure S and T computations are complete before M computations start using them
    ...
    sgemm_strassen_1level_fused_vP(m2,
                                   n2,
                                   k2,
                                   alpha_inner,
                                   S0_d,
    ...
    // Cleanup
    CUDA_CHECK(cudaFreeAsync(S0_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(S1_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(S4_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(S5_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(S6_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(T0_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(T2_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(T3_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(T5_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(T6_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(M0_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(M1_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(M2_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(M3_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(M4_d, main_stream));
    CUDA_CHECK(cudaFreeAsync(M5_d, main_stream));
